...
class BreadthMonitor:
    ...
    def analyze(self, prices=None):
        ...
        # Check if RSP and SPY are in columns
        if "RSP" not in prices.columns or "SPY" not in prices.columns:
            raise ValueError("No RSP or SPY tickers available in price data.")
        ...
        # Calculate Z-Score relative to 1-year history (252 trading days)
        rolling_mean = ratio.rolling(window=252).mean()
        rolling_std = ratio.rolling(window=252).std()
        z_score = (ratio - rolling_mean) / rolling_std
        ...
        if current_z > 1.0:
            ...
            regime = "Moderate Breadth"
            color = "#aaffaa"
        elif current_z > -1.0:
            regime = "Narrowing Breadth"
            color = "#ffaaaa"
        else:
            regime = "Extreme Concentration Risk"
            color = "#ef4444"
    ...
    def create_plot(self, results):
        ...
        fig.update_layout(
            title=dict(
                text="Market Breadth Monitor (RSP Equal-Weight vs. SPY Cap-Weight)",
                font=dict(size=20, color="#f8fafc")
            ),
            template="plotly_dark",
            paper_bgcolor='rgba(0,0,0,0)',
